...
| `vllm_stacked_container_triton_cache_invalidate` | `false` | Before `vllm serve`: wipe `~/.triton/cache` inside each Ray container (GB10 sm_121 stale-cache garble — vllm#41871). Defaulted **off** since 2026-05-27 — Triton cache reuse cuts ~30–90 s off the 35B FP8 MoE warmup and we no longer observe stale-cache garble on NGC 25.11+ / vLLM 0.17.1+. Flip to `true` if you suspect kernel mis-compilation after an image, driver, or GPU arch bump. |
